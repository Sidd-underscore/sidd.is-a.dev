...
jQuery(document).ready(function($) {
    ...
    if (index != -1) {
        ...
        $("#hero").addClass("destroy").css({
            'display': 'none'
        });
        $('body').toggleClass("work-mode");
    ...
    $(document).on('click', 'body:not(".work-mode") .view-case-study', function(e) {
        ...
        window.location.href = siteUrl + projectUrl;
    ...
    function loadPage(url, prettyUrl) {
        $("#project-inner-container").load(url, function(response, status, xhr) {
            if (status == "success") {
                ...
                var currentProject = $('#entry-slug').val();
                $('body').addClass("work-mode");
                $('.view-case-study').removeClass("loader");
            }
        });
    }
    function loadPage2(url, prettyUrl) {
        var currentProject = $('#entry-slug').val();
        $('#project-inner-container').animate({
            'opacity': '0'
        }, 350, function() {
        ...
        if (totalScroll > 400) {
            if ((fullPageInit == false) && (workPage == false)) {
                ...
                $('#hero').animate({
                    ...
                    loadedSection.find(".full-line").animate({
                        'width': '100%'
                    }, 500);
